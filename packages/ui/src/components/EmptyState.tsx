...
export const EmptyState = React.forwardRef<HTMLDivElement, EmptyStateProps>(
  ({ className, title, description, icon: Icon, action, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('flex flex-col items-center gap-3 rounded-(--radius-lg) border border-dashed border-(--color-border) p-10 text-center', className)}
      {...props}
    >
      ...
      <div className="flex flex-col gap-1">
        ...
        {description ? <p className="text-(--text-sm) text-(--color-muted)">{description}</p> : null}
      </div>

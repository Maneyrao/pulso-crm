...
| `#/access` | `/access` | `POST /access/check`, `GET /access/attempts` | ✅🚧 | Input DNI grande + REGISTRAR, feed en vivo real desde `/access/attempts`, grid de hardware con **estado real del agente** (fase 4). Sin "lector facial"/"barrera" (no existen → no se muestran) |
...
3. Web: cliente WS local (`hello`→`hello.ack`, ping 15s, backoff con jitter, Zod en cada mensaje entrante, `deviceToken` nunca persistido), badge de estado en topbar, enrolamiento desde ficha del socio (tab Biometría con consentimiento previo), identificación continua en `/access`, `/settings/devices` real.
...
| Fase 4 fingerprint | ✅ (núcleo) `apps/local-agent` .NET 8 completo (Protocol/Core/Sensors/Backend/Host + FakeSensor, 126 tests). `packages/contracts/src/agent-protocol.ts` hecho: espejo Zod de MessageCodec validado contra las mismas fixtures que el lado C# (81 tests, §11 del protocolo). `packages/contracts/src/biometrics.ts` hecho: superficie CRM (agents/devices/consent/enrollments/credentials, sólo metadatos — el schema de credencial se testea para NO exponer campos de template) + superficie agente espejo de `HttpDtos.cs` (pair/heartbeat/events/enroll-complete/identify `{resolved:true}`), 13 tests. Backend completo: Prisma (LocalAgent→DeviceToken + TenantBiometricKey, uniques parciales en SQL), módulo Nest `agents` (CRM device:manage + superficie `/agent/*` con pareo secreto-único y credencial Bearer; AuthGuard resuelve `@AgentOnly()`) y módulo `biometrics` (consent/enrollments/credentials + enroll-complete/identify; sobre AES-256-GCM con AAD `gymId||credentialId||keyVersion`, KEK por tenant envuelta por MASTER_KEK; matcher pluggable con igualdad de bytes hasta el SDK real; identify reusa `AccessService.checkForMember` — la misma cadena de autorización de /access/check). 15 tests de seguridad §13 en `test/biometrics/biometrics.spec.ts` (consent 409, token scope/replay/atado a sesión, revocación inmediata, cascada, AAD cross-tenant, cross-branch, cross-tenant CRM, no-PII, pareo). Web hecho (2026-08-22): cliente WS real `lib/agent/real-agent.ts` sobre `parseAgentMessageJson` (hello→hello.ack, ping 15 s, backoff+jitter, deviceToken sólo en memoria; 5 tests con WebSocket falso), `lib/api/biometrics.ts`, `/settings/devices` real (agentes/lectores por API, alta con secreto mostrado UNA vez, approve/revoke, badge de estado del agente local), tab Biometría en perfil de socio (consentimiento, credenciales sólo-metadatos, enrolamiento end-to-end vía agente) y `EnrollmentDialog` real. El fake-agent queda como doble de test del mismo contrato (`lib/agent/client.ts`). Pendiente Etapa 8: endpoint CRM para emitir deviceTokens IDENTIFY (hoy sólo los emite el flujo de enrolamiento; los tests de identify los crean directo), identify continuo en `/access` (la pantalla ya muestra los intentos por polling), matcher real con SDK DigitalPersona, push WS `access.resolved` | |

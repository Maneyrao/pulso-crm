...
| Fase 4 fingerprint | 🔶 `apps/local-agent` .NET 8 completo (Protocol/Core/Sensors/Backend/Host + FakeSensor, 126 tests). `packages/contracts/src/agent-protocol.ts` hecho: espejo Zod de MessageCodec validado contra las mismas fixtures que el lado C# (81 tests, §11 del protocolo). Pendiente: `biometrics.ts` (consent/enrollment/credential), módulos Nest `agents`/`biometrics` + Prisma, cliente WS web sobre `parseAgentMessageJson`, `/settings/devices` real | |

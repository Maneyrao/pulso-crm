...
| **Reglas de autorización, en orden** | 1. Buscar socio por documento normalizado o tarjeta, dentro del `gymId`. Si no hay → `DENIED/MEMBER_NOT_FOUND`. <br>2. `Member.status != ACTIVE` → `DENIED/MEMBER_INACTIVE`. <br>3. Sin membresía → `DENIED/NO_MEMBERSHIP`. <br>4. Membresía cancelada o vencida → `DENIED/MEMBERSHIP_CANCELLED` o `DENIED/MEMBERSHIP_EXPIRED`. <br>5. `Member.balance < 0` → `DENIED/DEBT_BLOCKED`; aplica también a huella y no registra asistencia. <br>6. Plan no habilitado en esa sede → `DENIED/BRANCH_NOT_ALLOWED`. <br>7. `classesRemaining = 0` → `DENIED/NO_CLASSES_REMAINING`. <br>8. Si ya hay asistencia hoy → `ALLOWED/DUPLICATE_WINDOW`, **sin** crear una segunda asistencia ni descontar clase. <br>9. `ALLOWED/OK`: crear `Attendance` y decrementar `classesRemaining` si el plan es por clases. |

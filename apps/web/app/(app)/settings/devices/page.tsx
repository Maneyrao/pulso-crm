...
const AGENT_STATUS_BADGE: Record<
  LocalAgent['status'],
...
const DEVICE_STATUS_BADGE: Record<
  AccessDevice['status'],
  { label: string; tone: 'success' | 'warning' | 'neutral' | 'danger' }
> = {
...
function DevicesScreen() {
  ...
  const [pairing, setPairing] = React.useState<{ secret: string; installationId: string } | null>(
    null,
  );
  ...
  const revokeMutation = useMutation({
    mutationFn: (input: { id: string; reason: string }) =>
      revokeAgent(input.id, { reason: input.reason }),
    onSuccess: () => {
      toast({
        title: 'Agente revocado',
        description: 'Sus tokens en curso quedaron invalidados.',
        tone: 'success',
      });
      setToRevoke(null);
      setRevokeReason('');
  ...
  const agentColumns: DataTableColumn<LocalAgent>[] = [
    {
      id: 'name',
      header: 'Agente',
      cell: (a) => <span className="font-medium text-(--color-text)">{a.name}</span>,
    },
    { id: 'branch', header: 'Sede', cell: (a) => branchName(a.branchId) },
    {
      id: 'version',
      header: 'Versión',
      cell: (a) => <span className="font-mono text-(--text-xs)">{a.agentVersion ?? '—'}</span>,
    },
    {
      id: 'lastSeen',
  ...
  const deviceColumns: DataTableColumn<AccessDevice>[] = [
    {
      id: 'model',
      header: 'Lector',
      cell: (d) => <span className="font-medium text-(--color-text)">{d.model}</span>,
    },
    { id: 'branch', header: 'Sede', cell: (d) => branchName(d.branchId) },
    {
      id: 'serial',
      header: 'Serie',
      cell: (d) => <span className="font-mono text-(--text-xs)">{d.serialNumber ?? '—'}</span>,
    },
    {
      id: 'status',
      header: 'Estado',
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Conexión WebSocket local (wss://127.0.0.1:21987).{' '}
            {deviceName ? `Lector: ${deviceName}.` : ''}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex items-center gap-3">
          <StatusBadge
            tone={status === 'ready' ? 'success' : status === 'no-agent' ? 'neutral' : 'warning'}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Un agente por PC de recepción. La revocación invalida sus tokens al instante.
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        {pairing ? (
          <div className="space-y-4">
            <Alert tone="warning" title="Se muestra una sola vez">
              Copiá estos valores en la instalación de El Templo Agent. Al cerrar este diálogo no se
              pueden recuperar.
            </Alert>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              onClick={() =>
                toRevoke && revokeMutation.mutate({ id: toRevoke.id, reason: revokeReason })
              }
            >
              Revocar
            </Button>
